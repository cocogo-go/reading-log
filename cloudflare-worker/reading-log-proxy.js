// 정보나루(data4library.kr) API 중계 Worker.
//
// 클라이언트(reading-log 앱)는 이 Worker를 통해 정보나루 API를 호출한다.
// 실제 인증키(authKey)는 여기서만 사용하는 서버 쪽 비밀(env.DATA4LIBRARY_KEY)이며,
// 클라이언트 코드나 응답에는 절대 노출하지 않는다.
//
// 배포: Cloudflare 대시보드 > Workers & Pages > 이 스크립트를 붙여넣고,
// Settings > Variables에 DATA4LIBRARY_KEY를 Secret으로 등록한다.
//
// 호출 형식: GET /api/<endpoint>?<정보나루 파라미터, authKey/format 제외>
...
export default {
  async fetch(request, env) {
    ...
    if (!match || !ALLOWED_ENDPOINTS.has(match[1])) {
